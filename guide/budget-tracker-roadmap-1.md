...
**Goal of this phase:** the two v1 notification types, delivered as real
remote push via the Expo Push Service, both of which lean on the
calculation you just finished.
...
- [ ] **Session 21.5: Set up a Firebase project and create an EAS Build
  Android development build.** Install it on your phone in place of Expo
  Go. *Why here:* Expo Go no longer supports remote push on Android, so
  this has to happen before push notifications can be tested at all —
  worth doing as its own session since it's setup, not app logic.
- [ ] **Session 22: Daily reminder job.** A scheduled task (using the
  external-scheduler workaround from the documentation) that calls the
  daily-summary logic each morning per budget and sends a push
  notification via the Expo Push Service. *Why:* your first taste of
  background jobs, distinct from request/response endpoints.
...
backend. Android only — use the EAS Build development build from Session
21.5 for testing rather than Expo Go once you're past the notification
sessions, so push notifications keep working throughout the rest of
development.
