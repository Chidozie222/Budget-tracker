...
| Mobile framework detail | Expo (managed workflow) | Simplifies push notifications — no Firebase/native cert setup needed for the base setup |
| Platform target | Android only | Avoids the $99/year Apple Developer Program cost required for iOS push notifications |
| Mobile build for testing | EAS Build (development build) | Native build installed directly on the phone — required because Expo Go (SDK 53+) no longer supports remote push on Android |
...
| Push notifications | Expo Push Service, via an EAS Build development build | Real remote push (server-triggered) — Expo Go no longer supports this on Android (SDK 53+). Requires a free Firebase project for Android FCM credentials. Android-only, so no Apple Developer cost applies |
...
Two notification types chosen for v1, kept intentionally minimal, delivered
as **real remote push notifications** via the Expo Push Service:

1. **Daily reminder of today's recommended spend** — a scheduled backend
   job calls the daily-summary calculation and sends a push notification
   with the number to the user before they start spending.
2. **Reminder to decide save-or-carry-forward on underspend** — triggered
   when the daily-summary calculation detects an underspend the user
   hasn't yet acted on, sent as a push notification prompting the choice
   from Rule 9.

**Platform scope:** Android only. Testing requires an **EAS Build
development build** installed on the phone, not Expo Go — Expo Go (SDK 53+)
no longer supports remote push on Android. This needs a free Firebase
project for Android FCM credentials; no Apple Developer cost applies since
iOS isn't targeted.

Because these are server-triggered, the backend still needs a way to run
the check on a schedule — see the external-scheduler entry in the
Low-Level Tooling table (Section 5), which works around Render's free-tier
